...
export default function ChannelList({ loading = false }) {
  ...
  const { filteredChannels, handleFilterChange, channels, activeType } = useChannelFilter();
  useTypingIndicator();

  if (!user) return null;

  const emptyChannelHead = activeType === "threads" ? "No Threads Found" : "No Channels Found";
  const emptyChannelList =
    activeType === "threads"
      ? "It looks like there are no threads available."
        ...
        <div className=" mt-2 h-svh overflow-y-auto">
          {filteredChannels?.length === 0 && (
            <div className="flex size-full flex-col items-center justify-center overflow-hidden">
              <span className="font-semibold antialiased">{emptyChannelHead}</span>
              <span className="my-4 text-center text-sm">{emptyChannelList}</span>
              {activeType !== "threads" && <NewChannelModal />}
            </div>
          )}
